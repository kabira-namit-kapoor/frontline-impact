...
putdocx text ("Number of valid observations = `N'")
	...
	* table
	...
	table () () [aw=sample_wt], stat(mean $ml) nformat(%6.2f)
	...
	stat(column) include sort(1) descending ///
	blabel(bar, format(%8.3g) pos(out) size(vsmall))  ///
	`graphopts' ///
...
table () () [aw=sample_wt], stat(mean `varlist') nformat(%6.2f)
collect style putdocx, layout(autofitcontents)
putdocx collect 
